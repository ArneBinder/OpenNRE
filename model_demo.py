    ...
    def __init__(self, train_data_loader, batch_size, max_length=120):
        ...
        if model.selector == "att":
            ...
            self._train_logit, train_repre = nrekit.network.selector.bag_one(x_train, self.scope, self.label,
                                                                             self.rel_tot, True, keep_prob=0.5)
            self._test_logit, test_repre = nrekit.network.selector.bag_one(x_test, self.scope, self.label, self.rel_tot,
                                                                           False, keep_prob=1.0)
            self._test_logit = tf.nn.softmax(self._test_logit)
        elif model.selector == "cross_max":
            self._train_logit, train_repre = nrekit.network.selector.bag_cross_max(x_train, self.scope, self.rel_tot,
                                                                                   keep_prob=0.5)
...
def get_name(dataset, encoder, selector, use_prepared_embeddings):
    return dataset + "_" + encoder + "_" + selector + '_pe' + str(use_prepared_embeddings)