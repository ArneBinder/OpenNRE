...
class json_file_data_loader(file_data_loader):
    ...
    def _load_preprocessed_file(self):
        ...
        dataset_dir = os.path.dirname(self.file_name)
        processed_data_dir = os.path.join(dataset_dir, '_processed_data')
        ...
        if not os.path.exists(word_npy_file_name) or \
           ...
           not os.path.exists(word_vec_mat_file_name) or \
                (self.add_embeddings and not os.path.exists(embedding_npy_file_name)) or \
    ...
    def __init__(self, file_name, word_vec_file_name, rel2id_file_name, mode, shuffle=True, max_length=120, case_sensitive=False, reprocess=False, batch_size=160, add_embeddings=False):
        '''
